    ...
    },
    {
      ...
      subitems: [
        ...
        },
        {
          label: "expense",
          to: "react/route",
          icon: "./assets/icons/remove-circle-outline.svg",
        },
      ],
      active: false,
    },
...
export const GlobalProvider = ({ children }) => {
  ...
  async function handleNewRecord(typeofRecord) {
    ...
    try {
      const res = await axios.post(
        "https://budbud-7tc8.onrender.com/api/v1/records/new",
        state.newRecord,
        config
      );
      dispatch({
  ...
  async function handleRecordDelete(recordId) {
    try {
      const res = await axios.delete(
        `https://budbud-7tc8.onrender.com/api/v1/records/delete/${recordId}`
      );
      dispatch({
        type: "RECORD_DELETE",
        payload: recordId,
  ...
  async function getLatestRecords(count = 10) {
    try {
      const res = await axios.get(
        `https://budbud-7tc8.onrender.com/api/v1/records/latest/${count}`
      );
  ...
  async function updateBalance() {
    try {
      const res = await axios.get(
        "https://budbud-7tc8.onrender.com/api/v1/records/get/all"
      );
  ...
  async function handleRecordUpdate() {
    ...
    try {
      const res = await axios.put(
        `https://budbud-7tc8.onrender.com/api/v1/records/update/${state.newRecord.record_id}`,
        state.newRecord,
        config
      );
      dispatch({
  ...
  async function handleListFilterChange({ target }) {
    try {
      ...
      switch (filter) {
        case "latest10":
          res = await axios.get(
            "https://budbud-7tc8.onrender.com/api/v1/records/latest/10"
          );
          break;
        ...
        case "earnt":
          res = await axios.get(
            "https://budbud-7tc8.onrender.com/api/v1/records/type/earnt"
          );
          break;

        case "spent":
          res = await axios.get(
            "https://budbud-7tc8.onrender.com/api/v1/records/type/spent"
          );
          break;
